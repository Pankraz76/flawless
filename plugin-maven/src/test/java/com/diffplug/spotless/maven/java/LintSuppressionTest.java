...
class LintSuppressionTest extends MavenIntegrationHarness {
	...
	void testNoSuppressionFailsOnWildcardImports() throws Exception {
		...
		expectSelfieErrorMsg(mavenRunner().withArguments("spotless:check").runHasError()).toBe("""
				Failed to execute goal com.opencohesion.flawless:flawless-maven-plugin:VERSION:check (default-cli) on project flawless-maven-plugin-tests: Unable to format file ${PROJECT_DIR}/src/main/java/TestFile.java
